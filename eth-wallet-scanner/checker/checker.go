...
type Scanner struct {
        ...
        // lastIdx — index terakhir yang sudah dikirim ke worker (diupdate oleh producer)
        lastIdxMu sync.Mutex
        lastIdx   *big.Int
}

// updateLastIdx menyimpan index tertinggi yang sudah didispatch ke worker
...
// LastIndex mengembalikan salinan index terakhir yang sudah diproses (thread-safe)
        ...
        go func() {
                ...
                for current.Cmp(endIndex) <= 0 {
                        ...
                        if len(batch) == batchSize {
                                // Catat index terakhir dalam batch sebelum dikirim
                ...
                // Catat index terakhir dari sisa batch
                if len(batch) > 0 {
                        s.updateLastIdx(batch[len(batch)-1])
                }
                flush()
